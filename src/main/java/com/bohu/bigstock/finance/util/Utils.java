...
public class Utils {
    ...
    public static Calendar parseDividendDate(String date) {
        ...
        date = date.trim();
        SimpleDateFormat format = new SimpleDateFormat(Utils.getDividendDateFormat(date), Locale.CHINA);
        format.setTimeZone(TimeZone.getTimeZone("GMT+08:00"));
        try {
            Calendar today = Calendar.getInstance(TimeZone.getTimeZone("GMT+08:00"));
            Calendar parsedDate = Calendar.getInstance(TimeZone.getTimeZone("GMT+08:00"));
    ...
    public static Calendar parseDateTime(String date, String time, TimeZone timeZone) {
        ...
        SimpleDateFormat format = new SimpleDateFormat("M/d/yyyy h:mma", Locale.CHINA);
    ...
    public static Calendar parseDateStringToCalendar(String date,String parttern) {
        SimpleDateFormat format = new SimpleDateFormat(parttern, Locale.CHINA);
        try {
            if (Utils.isParseable(date)) {
                Calendar c = Calendar.getInstance();
                c.setTime(format.parse(date));
    ...
    public static String calendarToDateString(Calendar cal, String pattern) {
        SimpleDateFormat formatter = new SimpleDateFormat(pattern);
        String DateStr = formatter.format(cal.getTime());
        return DateStr;
    }

    public static BigDecimal getSinaPriceViberation(String vibration) {
        if ("--".equalsIgnoreCase(vibration)) return BigDecimal.ZERO;
        return getBigDecimal(vibration);
    }
